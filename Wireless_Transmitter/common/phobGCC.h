...
//**********JLS2024: currently, only phob 2.0x is supported for the wireless project. 
#include "../include/Phob2_0.h"           // For PhobGCC Board 2.0 with RP2040
...
void processButtons(Pins &pin, Buttons &btn, Buttons &hardware, ControlConfig &controls, FilterGains &gains, FilterGains &normGains, int &currentCalStep, bool &running, float tempCalPointsX[], float tempCalPointsY[], WhichStick &whichStick, NotchStatus notchStatus[], float notchAngles[], float measuredNotchAngles[], StickParams &aStickParams, StickParams &cStickParams){
	...
	// heather funny controls toggle.

	if (!gpio_get(15)) 
	{
		tempBtn.B = hardware.X;
		tempBtn.X = 0;
	}
...
//#ifndef CLEANADC
	...
	do{
		monitorTransmission(adcCount); // give it adccount because for the first few we want it to do nothing.
		adcCount++;
		aXSum += readAx(pin);
		aYSum += readAy(pin);
		cXSum += readCx(pin);
	...
	*/
	...
	//printf("\n%.4f | %.4f", raw.cxRaw, raw.cyRaw);

	if (!gpio_get(15))
	{
		// forbid Cy from moving > 0 (which is 128) under normal circumstances...
		btn.Cy = (btn.Cy > 128) ? 128 : btn.Cy;
		if (hardware.B)
		{
			btn.Cy += 127;
		}
