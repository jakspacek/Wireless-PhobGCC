	...
	*/
	...
	while(true) { //main event loop
		...
		if(_currentCalStep >= 0){
			...
			//gpio_put(12, _btn.S); // start button
			synchronousSubmitForWireless(_btn); // wireless.h
		}
		else
		{
...
int main() {
	...
	gpio_init(CSN_PIN__TX);
    gpio_set_dir(CSN_PIN__TX, GPIO_OUT);
    gpio_init(CE_PIN__TX);
    gpio_set_dir(CE_PIN__TX, GPIO_OUT);
    //gpio_init(12);
    //gpio_set_dir(12, GPIO_OUT); //latency measurement.
	...
	gpio_init(15); // funny controls toggle.
	...
	if(_hardware.Z) { //hold Z on powerup for PhobVision
		...
		_controls.funnytoggle = gpio_get(15);

		while (true)
		{
			sleep_ms(1000);
			memory_dump_to_screen();
			printf("\nfunny toggle says %d", _controls.funnytoggle);
		}
	}
	...
	while (true) {
		...
		// i think i should just call the second_core loop here.
